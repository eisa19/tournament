...
var gamesInSession = [];
var tournamentLobby = ["not changed"];
...
function playerJoinsTournament(tidData) {
  const username = tidData.userName;
  // store players that join the tournament lobby
  tournamentLobby.push(username);
  ...
  if (room.length < 4) {
    ...
    io.on("connection", (socket) => {
      ...
      const tournamentLobbyfun = (tournamentLobby) => {
        // Your function logic goes here
        x = tournamentLobby
        return x;
      };

      io.emit("tournament-lobby-data",tournamentLobby);
    });
  } else {
    // Otherwise, send an error message back to the player.
    this.emit("status", "There are already 2 people playing in this room.");
